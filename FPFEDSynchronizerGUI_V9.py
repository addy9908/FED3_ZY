...
class DataManager:
    ...
    def reset(self):
        self.raw_fp_files.clear()
        self.raw_cam_files.clear()
        self.raw_fed_l_files.clear()
        self.raw_fed_r_files.clear()
...
class Panel1_Browser(ttk.Frame):
    def __init__(self, parent, data_mgr):
        ...
        self.lb_fp = self.make_list(lf, "FP Files", self.data_mgr.raw_fp_files, 0)
        self.lb_cam = self.make_list(lf, "CAM Files", self.data_mgr.raw_cam_files, 1)
        self.lb_fed_l = self.make_list(lf, "FED L", self.data_mgr.raw_fed_l_files, 2)
        self.lb_fed_r = self.make_list(lf, "FED R", self.data_mgr.raw_fed_r_files, 3)
    ...
    def make_list(self, p, title, lst, col):
        ...
        def delete():
            """Removes the selected file from both the GUI listbox and the DataManager list."""
            sel = lb.curselection()
            if sel:
                idx = sel[0]
                lb.delete(idx)   # Remove from GUI
                lst.pop(idx)    # Remove from backend memory
        ...
        ttk.Button(f, text="- Del", command=delete).pack(pady=2)

        return lb

    def reset_ui(self):
        """Wipes the listboxes when the global reset is triggered."""
        self.lb_fp.delete(0, tk.END)
        self.lb_cam.delete(0, tk.END)
        self.lb_fed_l.delete(0, tk.END)
        self.lb_fed_r.delete(0, tk.END)
        self.tree.delete(*self.tree.get_children())
...
class Panel2_FPProcess(ttk.Frame):
    def __init__(self, parent, data_mgr):
        ...
        self.signal_init = self.mk_ent(f, "Signal Initial:", "G", 0)
        self.e470 = self.mk_ent(f, "470 Flag:", "2", 2)
        self.e415 = self.mk_ent(f, "415 Flag:", "1", 4)
        self.efilt = self.mk_ent(f, "Filtfilt Window:", "4", 6)
        self.epoly = self.mk_ent(f, "Polyfit Power:", "2", 8)
        
        bf = ttk.Frame(self); bf.pack(pady=5)
        ttk.Button(bf, text="1. Preview Raw", command=self.preview).pack(side=tk.LEFT, padx=10)
        
        ttk.Label(bf, text="X-Axis Unit:").pack(side=tk.LEFT, padx=5)
        self.x_axis_cb = ttk.Combobox(bf, values=["sec", "min", "hour"], width=8)
    ...
    def prepare(self):
        ...
        if current_files:
            print("The tuple is NOT empty! It has files.")
        else:
            print("The tuple IS empty!")
        if not current_files: return False
        if getattr(self, '_cached_files', None) != current_files:
            ...
            if cc in df.columns and sc in df.columns:
                # 1. Sort by SystemTimestamp to fix out-of-order file concatenations
                df = df.sort_values(by=sc).reset_index(drop=True)
                
                # 2. Calculate row-to-row differences
                cc_diff = df[cc].diff().fillna(0)
                sys_diff = df[sc].diff().fillna(0) * 1000.0  # Convert to ms
                
                # Force sys_diff to be at least 1ms forward to prevent flatlines
                sys_diff = np.maximum(sys_diff, 1.0)
                
                # 3. If cc drops/resets (diff <= 0), use sys_diff. Otherwise, trust cc.
                best_diff = np.where(cc_diff > 0, cc_diff, sys_diff)
                
                # Keep the original starting timestamp
                best_diff[0] = df[cc].iloc[0] if df[cc].iloc[0] > 0 else 0
                
                # 4. Rebuild the column using cumulative sum
                df[cc] = np.cumsum(best_diff)
        ...
        df = self._df_fp_raw; sigs = [c for c in df.columns if c.startswith(self.signal_init.get())]
    ...
    def preview(self):
        ...
        t = self.df_sig[self.data_mgr.time_col].iloc[::ds] * factor
        ...
        self.ax1.legend(fontsize='x-small'); self.ax1.set_xlabel(f"Time ({self.x_axis_cb.get()})"); self.ax1.set_title("Raw") 
    ...
    def process(self):
        ...
        ds = max(1, len(self.df_sig)//10000)
        factor = {"sec": 1.0, "min": 1/60.0, "hour": 1/3600.0}[self.x_axis_cb.get()]
        t = dff[self.data_mgr.time_col].iloc[::ds] * factor
        ...
        for c in [c for c in self.df_sig.columns if c.startswith("G")]:
            ...
            dff[c] = (sf - fit) / fit
            self.ax2.plot(t, dff[c].iloc[::ds], label=f"{c} dF/F")
               
        self.data_mgr.set_df_fp(dff)
        self.ax2.legend(fontsize='x-small'); self.ax2.set_xlabel(f"Time ({self.x_axis_cb.get()})")
        self.fig.tight_layout(); self.canvas.draw()

    def reset_ui(self):
        """Clears the plots and dropdowns when global reset is triggered."""
        self.fig.clf()               # Wipe the matplotlib figure
        self.ax1 = self.fig.add_subplot(211)
        self.ax2 = self.fig.add_subplot(212)
        self.canvas.draw()           # Redraw the empty canvas
        self._df_fp_raw, self.df_sig, self.df_ref_cal = None, None, None
        self._cached_files = None
...
class Panel3_CamProcess(ttk.Frame):
    def __init__(self, parent, data_mgr):
        ...
        self.canvas = FigureCanvasTkAgg(self.fig, master=self)
        NavigationToolbar2Tk(self.canvas, self).pack(fill="x"); self.canvas.get_tk_widget().pack(fill="both", expand=True)
...
class Panel4_FedProcess(ttk.Frame):
    def __init__(self, parent, data_mgr):
        ...
        # --- FED Info & Sync Frame ---
        f_sync = ttk.Frame(self)
        f_sync.pack(fill="x", pady=5)
        
        # Left FED Label
        self.lbl_start_time_L = ttk.Label(f_sync, text="FED L Start: --", font=("Helvetica", 10, "bold"), foreground="blue", width=50, anchor="w")
        self.lbl_start_time_L.pack(side=tk.LEFT, padx=20)
        
        # Right FED Label
        self.lbl_start_time_R = ttk.Label(f_sync, text="FED R Start: --", font=("Helvetica", 10, "bold"), foreground="green", width=50, anchor="w")
        self.lbl_start_time_R.pack(side=tk.LEFT, padx=20)
        
        ttk.Label(f_sync, text=" | Time offset to FP (s):").pack(side=tk.LEFT, padx=5)
        self.var_offset = tk.StringVar(value="0.0")
        self.e_offset = ttk.Entry(f_sync, textvariable=self.var_offset, width=8)
        self.e_offset.pack(side=tk.LEFT, padx=5)
        
        self.e_offset.bind("<KeyRelease>", lambda e: self.plot())
        # -----------------------------
    ...
    def _process_fed_files(self, files, label):
        ...
        if False:
            ...
            valid_idx = np.where(valid_fall_mask)[0]
            
            # --- NEW: CALCULATE START TIME WITH ABSOLUTE TIMESTAMP ---
            dio_name = label
            start_time_str = "Not Found"
            log_msg = f"WARNING: {dio_name} Start Time not found."
            log_type = "WARN"
    
            if len(valid_idx) > 0:
                # Primary logic: First valid 0.2s pulse
                first_fall_idx = valid_idx[0]
                start_row = max(0, first_fall_idx - 2) 
                start_time_str = str(df.iloc[start_row, 0])
                log_msg = f"{dio_name} Start Time: {start_time_str} (right before 0.2s Pulse)"
                log_type = "INFO"
                
            else:
                # Fallback logic: Look for short pulses (0.04s - 0.15s)
                fallback_mask = (dt >= 0.04) & (dt <= 0.15)
                fallback_idx = np.where(fallback_mask)[0]
                
                if len(fallback_idx) > 0:
                    last_fall_idx = fallback_idx[-1]
                    start_row = max(0, last_fall_idx - 2)
                    start_time_str = f"{df.iloc[start_row, 0]} (Fallback)"
                    log_msg = f"{dio_name} Start Time: {start_time_str} (Last 0.1s Pulse)"
                    log_type = "INFO"
    
            # --- ROUTE TO THE CORRECT GUI LABEL ---
            display_text = f"{dio_name[-1]} Start: {start_time_str} with total pellet {len(valid_idx)}" # e.g. "L Start: 2024-..."
            
            if dio_name == 'DIO_fed_L':
                self.lbl_start_time_L.config(text=display_text)
            elif dio_name == 'DIO_fed_R':
                self.lbl_start_time_R.config(text=display_text)
                
            self.data_mgr.log(log_msg, log_type)
            
            if len(valid_idx) > 1:
                median_gap = np.median(np.diff(valid_idx))
                if median_gap == 2.0:
                    self.data_mgr.log(f"{label}: Pulse width check passed: Valid events occur exactly every other row.", "INFO")
                else:
                    self.data_mgr.log(f"{label}: Pulse width note: Median row gap is {median_gap} (Expected 2). Noise was ignored.", "INFO")
            elif len(valid_idx) == 0:
                self.data_mgr.log(f"WARNING: No valid pulse widths (0.185s - 0.215s) found in {label} file!", "WARN")
                
            # Overwrite the target column 'c' with the cumulative count
            df.iloc[:, c] = valid_fall_mask.cumsum()
        # --------------------------------------------
    ...
    def plot(self):
        ...
        def _plot_ax(df, idx, label, ax):
            if df.empty: return
            try:
                # Safely grab the offset (if user types '-', it defaults to 0 until a number is typed)
                offset = float(self.var_offset.get())
            except ValueError:
                offset = 0.0
                
            # Add the offset to the time axis for the plot ONLY
            t_axis = df[self.data_mgr.time_col] + offset
            ax.clear(); ax.step(t_axis * factor, df.iloc[:,c])
    ...
    def merge(self):
        ...
        for df_fed, dio_name in zip([self.data_mgr.df_fed_L, self.data_mgr.df_fed_R], ['DIO_fed_L', 'DIO_fed_R']):
            ...
            if offset != 0.0:
                d_sub[self.data_mgr.time_col] = d_sub[self.data_mgr.time_col] + offset
                self.data_mgr.log(f"Applied {offset}s offset to FED '{dio_name}' before merging.", "INFO")
    ...
    def reset_ui(self):
        """Clears the plots and dropdowns when global reset is triggered."""
        self.fig.clf()               # Wipe the matplotlib figure
        self.lbl_start_time_L.config(text="FED L Start: --")
        self.lbl_start_time_R.config(text="FED R Start: --")
        self.ax1 = self.fig.add_subplot(211); self.ax2 = self.fig.add_subplot(212)
        self.canvas.draw()           # Redraw the empty canvas
...
class Panel8_Heatmap(ttk.Frame):
    def __init__(self, parent, data_mgr):
        ...
        self.cb_mode = ttk.Combobox(f_opt, values=["Raw dF/F", "Z-Score (Baseline)", "Z-Score (Full Trace)"], width=20)
        self.cb_mode.set("Z-Score (Baseline)")
        ...
        ttk.Label(f_auc, text="AUC1: Start(s):").pack(side=tk.LEFT, padx=2)
        self.e_auc_start1 = ttk.Entry(f_auc, width=5); self.e_auc_start1.insert(0, "-4"); self.e_auc_start1.pack(side=tk.LEFT, padx=2)
        
        ttk.Label(f_auc, text="End(s):").pack(side=tk.LEFT, padx=2)
        self.e_auc_end1 = ttk.Entry(f_auc, width=5); self.e_auc_end1.insert(0, "0"); self.e_auc_end1.pack(side=tk.LEFT, padx=2)
        
        self.lbl_auc_val_1 = ttk.Label(f_auc, text="AUC1: --", font=("Helvetica", 10, "bold"), foreground="purple")
        self.lbl_auc_val_1.pack(side=tk.LEFT, padx=5)
        
        ttk.Label(f_auc, text="AUC2: Start(s):").pack(side=tk.LEFT, padx=20)
        self.e_auc_start2 = ttk.Entry(f_auc, width=5); self.e_auc_start2.insert(0, "0"); self.e_auc_start2.pack(side=tk.LEFT, padx=2)
        
        ttk.Label(f_auc, text="End(s):").pack(side=tk.LEFT, padx=2)
        self.e_auc_end2 = ttk.Entry(f_auc, width=5); self.e_auc_end2.insert(0, "4"); self.e_auc_end2.pack(side=tk.LEFT, padx=2)
        
        self.lbl_auc_val_2 = ttk.Label(f_auc, text="AUC2: --", font=("Helvetica", 10, "bold"), foreground="purple")
        self.lbl_auc_val_2.pack(side=tk.LEFT, padx=5)
        
        ttk.Button(f_auc, text="Calculate AUCs", command=self.calculate_auc).pack(side=tk.LEFT, padx=20)
    ...
    def plot(self):
        ...
        try:
            ...
            self.lbl_auc_val_1.config(text="AUC1: --") # Reset AUC label on new plot
            self.lbl_auc_val_2.config(text="AUC2: --") # Reset AUC label on new plot
    ...
    def calculate_auc(self):
        if self.current_mean_sig is None or self.current_t_axis is None:
            self.lbl_auc_val_1.config(text="AUC1: Plot first!")
            self.lbl_auc_val_2.config(text="AUC2: Plot first!")
            return
            
        try:
            start1 = float(self.e_auc_start1.get())
            end1 = float(self.e_auc_end1.get())
            
            # Select only indices inside user window
            mask1 = (self.current_t_axis >= start1) & (self.current_t_axis <= end1)

            if not mask1.any():
                self.lbl_auc_val_1.config(text="AUC1: Invalid bounds")
                return
            x_win_1 = self.current_t_axis[mask1]
            y_win_1 = self.current_mean_sig[mask1]
            # Trapezoidal numerical integration
            auc_val1 = np.trapz(y_win_1, x_win_1)
            
            self.lbl_auc_val_1.config(text=f"AUC1: {auc_val1:.4f}")
            self.data_mgr.log(f"Calculated AUC1 ({start1}s to {end1}s): {auc_val1:.4f}", "INFO")
        
        except Exception as e:
            self.lbl_auc_val_1.config(text="AUC1: Calculation Error")
            self.data_mgr.log(f"AUC1 Error: {e}", "ERR")
        
        try:                        
            start2 = float(self.e_auc_start2.get())
            end2 = float(self.e_auc_end2.get())
            mask2 = (self.current_t_axis >= start2) & (self.current_t_axis <= end2)
            if not mask2.any():
                self.lbl_auc_val_2.config(text="AUC2: Invalid bounds")
                return
                
            x_win_2 = self.current_t_axis[mask2]
            y_win_2 = self.current_mean_sig[mask2]
            # Trapezoidal numerical integration
            auc_val2 = np.trapz(y_win_2, x_win_2)
            
            self.lbl_auc_val_2.config(text=f"AUC2: {auc_val2:.4f}")
            self.data_mgr.log(f"Calculated AUC2 ({start2}s to {end2}s): {auc_val2:.4f}", "INFO")
            
        except Exception as e:
            self.lbl_auc_val_2.config(text="AUC2: Calculation Error")
            self.data_mgr.log(f"AUC2 Error: {e}", "ERR")
...
class Panel9_Peak(ttk.Frame):
    def __init__(self, parent, data_mgr):
        ...
        ttk.Button(f_opt, text="2. Preview Global Peaks", command=self.preview_global).pack(side=tk.LEFT, padx=15)
        ttk.Button(f_opt, text="3. Plot Event-Aligned", command=self.plot_aligned).pack(side=tk.LEFT, padx=5)
...
class FPFEDApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("FP/FED Multi-Panel Synchronizer V8 by ZY")
        ...
        # clear data
        ttk.Button(bf, text="Clear All Data", command=self.clear_all_data).pack(side=tk.LEFT, padx=15)
    ...
    def clear_all_data(self):
        if messagebox.askyesno("Confirm Clear", "Are you sure you want to completely clear all loaded files and session data?"):
            
            # 1. Wipe the DataManager attributes
            self.data_mgr.reset()
            
            # 2. Clear the Treeview GUI
            self.tv.delete(*self.tv.get_children())
            ...
            for tab_id in self.nb.tabs():
                ...
                if hasattr(widget, 'reset_ui'):
                    widget.reset_ui()  # This will wipe listboxes in Panel 1, and plots in Panels 2-9
            
            self.log_message("All data cleared. System reset.", "INFO")
            
            # # 4. Optional: Force a refresh on the current active tab so it clears its plots
            # current_tab_id = self.nb.index("current")
            # current_tab_widget = self.nb.nametowidget(self.nb.tabs()[current_tab_id])
            # if hasattr(current_tab_widget, 'refresh_keys'):
            #     current_tab_widget.refresh_keys()
